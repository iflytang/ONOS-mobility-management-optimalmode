...
//    private static Ip4Address myIP = Ip4Address.valueOf("10.0.0.2");
    private static Ip4Address myIP = Ip4Address.valueOf("192.168.3.250");
...
//    private static Ip4Address broadcastAddress = Ip4Address.valueOf("10.255.255.255");
    private static Ip4Address broadcastAddress = Ip4Address.valueOf("192.168.3.255");

//    private static Ip4Address routerAddress = Ip4Address.valueOf("10.0.0.2");
    private static Ip4Address routerAddress = Ip4Address.valueOf("192.168.3.250");

//    private static Ip4Address domainServer = Ip4Address.valueOf("10.0.0.2");
    private static Ip4Address domainServer = Ip4Address.valueOf("192.168.3.250");
    ...
    protected void activate() {
        ...
        dhcpStore.populateIPPoolfromRange(Ip4Address.valueOf("192.168.3.3"), Ip4Address.valueOf("192.168.3.200"));
...
//                builder.setOutput(sourcePoint.port());
                ...
                int out_port = 1;
                DeviceId deviceId = DeviceId.deviceId("pof:0000000000000001");
...
//                            log.info(" send ACK in getIPAssignmentFromAllocationMap.");
                            sendReply(context, ethReply);
                            // ================ modification ====================
                            System.out.printf("DHCP server(AP1) assigned <deviceId: %s> <port: %d> < hwaddr: %s> <assignedIP: %s>.\n",
                                    deviceId,port,hwaddr,requestedIP);
                            log.info("DHCP server(AP1) assigned <[deviceId:{}] [port:{}] [hwaddr: {}] [assignedIP: {}]>.",
                                    deviceId,port,hwaddr, requestedIP);
...
//                                log.info("DHCPLease in DHCP-onos: deviceId:{}",deviceId);
//                                log.info("DHCPLease in DHCP-onos: hwaddr:{}",hwaddr);
//                                log.info("DHCPLease in DHCP-onos: port:{}",port);
//                                log.info("DHCPLease in DHCP-onos: coa:{}",requestedIP);
                            }
