...
public class DhcpManager implements DhcpService {
    ...
    private static final ProviderId PID = new ProviderId("pof", "org.test.dhcp", true);
    ...
    @Reference(cardinality = ReferenceCardinality.MANDATORY_UNARY)
    protected myDhcpStore dhcpStore;
    ...
    protected final ListenerRegistry<AssociationEvent, AssociationEventListener>
...
//    private static Ip4Address myIP = Ip4Address.valueOf("10.0.0.2");
    private static Ip4Address myIP = Ip4Address.valueOf("192.168.4.250");

    private static MacAddress myMAC = valueOf("3f:3f:3f:3f:3f:3f");
    ...
    private static int leaseTime = 600*6;
...
//    private static Ip4Address broadcastAddress = Ip4Address.valueOf("10.255.255.255");
    private static Ip4Address broadcastAddress = Ip4Address.valueOf("192.168.4.255");

//    private static Ip4Address routerAddress = Ip4Address.valueOf("10.0.0.2");
    private static Ip4Address routerAddress = Ip4Address.valueOf("192.168.4.250");

//    private static Ip4Address domainServer = Ip4Address.valueOf("10.0.0.2");
    private static Ip4Address domainServer = Ip4Address.valueOf("192.168.4.250");
    ...
    protected void activate() {
        ...
        appId = coreService.registerApplication("org.test.dhcp");

//        eventDispatcher.addSink(AssociationEvent.class, listenerRegistry);
        dhcpStore.populateIPPoolfromRange(Ip4Address.valueOf("192.168.4.3"), Ip4Address.valueOf("192.168.4.200"));
...
//        timer.schedule(new EventTimerTask(), 0, 3000);
    }
...
//        eventDispatcher.removeSink(AssociationEvent.class);
...
//    @Override
//    public void addListener(AssociationEventListener listener) {
//        listenerRegistry.addListener(listener);
//    }
//
//    @Override
//    public void removeListener(AssociationEventListener listener) {
//        listenerRegistry.removeListener(listener);
//    }

//    @Override
//    public EventDeliveryService getEventDispatcher() {
...
//            log.info("=============== come in buildReply =========");

            ipAssignment = dhcpStore.getIpAssignmentFromAllocationMap(HostId.hostId(packet.getSourceMAC()));
//            log.info("[buildReply: ipAssignment: {}, and !=null?:{}",ipAssignment, ipAssignment!= null);
//            log.info("[buildReply: ipAssignment.rangeNotEnforced: {}",ipAssignment.rangeNotEnforced());
...
//                log.info("[buildReply:(1) subnetMaskReply: {}, dhcpServerReply: {}, routerAddressReply: {}, domainServerReply: {}",
//                        subnetMaskReply,dhcpServerReply, routerAddressReply, domainServerReply);
            } else {
                subnetMaskReply = subnetMask;
                dhcpServerReply = myIP;
                routerAddressReply = routerAddress;
                domainServerReply = domainServer;
//                log.info("[buildReply:(2) subnetMaskReply: {}, dhcpServerReply: {}, routerAddressReply: {}, domainServerReply: {}",
//                        subnetMaskReply,dhcpServerReply, routerAddressReply, domainServerReply);
            }
...
//                builder.setOutput(sourcePoint.port());
                ...
                int out_port = 1;
                DeviceId deviceId = DeviceId.deviceId("pof:0000000000000002");
                List<OFAction> actions = new ArrayList<OFAction>();
                actions.add(DefaultPofActions.output((short) 0, (short) 0, (short) 0,(int) out_port).action());
                builder.add(DefaultPofInstructions.applyActions(actions));
//                log.info("DHCP: send reply to port <{}> using pofAction.", out_port);
//                log.info("DHCP: send reply to deviceId <{}> using pofAction.", sourcePoint.deviceId());
        ...
        private void processDhcpPacket(PacketContext context, DHCP dhcpPayload) {
            ...
            if(deviceId.equals("pof:0000000000000001")){
                return;  // only work for pof:2
            }
            log.info("222222222222222222222222222222222222");
//            int port = (int) inboundPacket.receivedFrom().port().toLong();
            int port = 1; // Ap1 and Ap2 output port to ues
            String hwaddr = inboundPacket.parsed().getSourceMAC().toString();  // src_Mac
//            log.info("DHCP: process DhcpPacket from <DeviceId: {}, port: {}, hwaddr: {}", deviceId, port, hwaddr);
...
//            log.info("[ sendReply: dhcpPayload !null?:{} ]",dhcpPayload!=null);
            if (dhcpPayload != null) {
...
//                        log.info("=====from dhcpPayload: incomingPacketType:{} ",incomingPacketType);
                    }
                    if (option.getCode() == DHCP.DHCPOptionCode.OptionCode_RequestedIP.getValue()) {
                        byte[] data = option.getData();
                        requestedIP = Ip4Address.valueOf(data);
//                        log.info("from dhcpPayload: requestIP == {}", requestedIP);
...
//                        log.info("from dhcpPayload: serverIP == {}", serverIP);
                        flagIfServerIP = true;
                    }
                }
...
//                            log.info(" send ACK in getIPAssignmentFromAllocationMap.");
                            sendReply(context, ethReply);
                            // ================ modification ====================
                            System.out.printf("DHCP server(AP2) assigned <deviceId: %s> <port: %d> < hwaddr: %s> <assignedIP: %s>.\n",
                                    deviceId,port,hwaddr,requestedIP);
                            log.info("DHCP server(AP2) assigned <[deviceId:{}] [port:{}] [hwaddr: {}] [assignedIP: {}]>.",
                                    deviceId,port,hwaddr, requestedIP);
                            // raise DHCPLease event
                            if(requestedIP.toString().equals("0.0.0.0")){
                                return;
                            }else{
                                AssociationEvent DHCPLease = new AssociationEvent(AssociationEvent.Type.DHCPLease, "DHCPLease",
                                    hwaddr, deviceId, port, requestedIP.toString());
                                eventDispatcher.post(DHCPLease);
//                                log.info("*******************************");
//                                log.info("[raise DHCPLease event in <SELECTING state in if>]");
//                                log.info("DHCPLease in DHCP-app: deviceId:{}",deviceId);
//                                log.info("DHCPLease in DHCP-app: hwaddr:{}",hwaddr);
//                                log.info("DHCPLease in DHCP-app: port:{}",port);
//                                log.info("DHCPLease in DHCP-app: coa:{}",requestedIP);
                            }
                                ...
                                if (dhcpStore.assignIP(hostId, requestedIP, leaseTime, false, Lists.newArrayList())) {
                                    ...
                                    if(requestedIP.toString().equals("0.0.0.0")){
                                        ...
                                        System.out.printf("DHCP server(AP2) assigned <deviceId: %s> <port: %d> < hwaddr: %s> <assignedIP: %s>.\n",
                                                deviceId,port,hwaddr,requestedIP);
                                        log.info("DHCP server(AP2) assigned <[deviceId:{}] [port:{}] [hwaddr: {}] [assignedIP: {}].",
                                                deviceId,port,hwaddr);
                                        // raise DHCPLease event
                                        AssociationEvent DHCPLease = new AssociationEvent(AssociationEvent.Type.DHCPLease, "DHCPLease",
                                                hwaddr, deviceId, port, requestedIP.toString());
                                        eventDispatcher.post(DHCPLease);
//                                        log.info("*******************************");
////                                        log.info("[raise DHCPLease event in <SELECTING state in else>]");
//                                        log.info("DHCPLease in DHCP-app: deviceId:{}",deviceId);
//                                        log.info("DHCPLease in DHCP-app: hwaddr:{}",hwaddr);
//                                        log.info("DHCPLease in DHCP-app: port:{}",port);
//                                        log.info("DHCPLease in DHCP-app: coa:{}",requestedIP);
...
//                                    log.info("[   DHCP_NACK in Selecting state ]");
                                }
                                Ethernet ethReply = buildReply(packet, requestedIP,
                                        (byte) outgoingPacketType.getValue());
//                                log.info(" send reply when myIp equals serverIP");
                                sendReply(context, ethReply);
                            }
                        }
                    } else if (flagIfRequestedIP) {
                        // INIT-REBOOT state
//                        log.info(" in flagIfRequestIp, requestedIP: {} ", requestedIP);
//                        log.info("dhcpStore.assignIP(hostId, requestedIP, leaseTime, false, Lists.newArrayList() : {})",
//                                dhcpStore.assignIP(hostId, requestedIP, leaseTime, false, Lists.newArrayList()));

                        if (dhcpStore.assignIP(hostId, requestedIP, leaseTime, false, Lists.newArrayList())) {
                            outgoingPacketType = DHCPPacketType.DHCPACK;
                            Ethernet ethReply = buildReply(packet, requestedIP, (byte) outgoingPacketType.getValue());
//                            log.info("send ACK in flagIfRequestIp");
                            sendReply(context, ethReply);
                            // ================ modification ====================
                            if(requestedIP.toString().equals("0.0.0.0")){
                                return;
                                ...
                                eventDispatcher.post(DHCPLease);
//                                log.info("*******************************");
//                                log.info("[raise DHCPLease event in <INIT-REBOOT state>]");
//                                log.info("DHCPLease in DHCP-app: deviceId:{}",deviceId);
//                                log.info("DHCPLease in DHCP-app: hwaddr:{}",hwaddr);
//                                log.info("DHCPLease in DHCP-app: port:{}",port);
//                                log.info("DHCPLease in DHCP-app: coa:{}",requestedIP);
...
//                        log.info(" in RENEWing state");
                        int ciaadr = dhcpPayload.getClientIPAddress();
//                        log.info("[RENEWING--ciaddr_int: {}", ciaadr);
                        if (ciaadr != 0) {
                            ...
                            if (dhcpStore.assignIP(hostId, clientIaddr, leaseTime, false, Lists.newArrayList())) {
                                ...
                                if(requestedIP.toString().equals("0.0.0.0")){
                                    ...
                                    eventDispatcher.post(DHCPLease);
//                                    log.info("*******************************");
//                                    log.info("[raise DHCPLease event in <RENEWING and REBINDING state>]");
//                                    log.info("DHCPLease in DHCP-app: deviceId:{}",deviceId);
//                                    log.info("DHCPLease in DHCP-app: hwaddr:{}",hwaddr);
//                                    log.info("DHCPLease in DHCP-app: port:{}",port);
//                                    log.info("DHCPLease in DHCP-app: coa:{}",requestedIP);
...
//                        log.info("RELEASE ip in DHCPRELEASE");
                    }
                }
            }
        }
            ...
            if (packet.getEtherType() == Ethernet.TYPE_IPV4) {
                ...
                if (ipv4Packet.getProtocol() == IPv4.PROTOCOL_UDP) {
                    ...
                    if (udpPacket.getDestinationPort() == UDP.DHCP_SERVER_PORT &&
                        ...
                        String deviceId = context.inPacket().receivedFrom().deviceId().toString();
                        if(deviceId.equals("pof:0000000000000002")) {
                            // only work for pof:2
...
//        public void run() {
//            AssociationEvent DHCPLease = new AssociationEvent(AssociationEvent.Type.DHCPLease, "DHCPLease",
//                    "pofswitch", "pof:0001", 1, "10.0.0.1");
//            eventDispatcher.post(DHCPLease);
//            log.info("Post AssociationEvent: {}", DHCPLease);
//        }
//    }
}
